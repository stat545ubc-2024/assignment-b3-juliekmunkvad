...
#Prepare the dataset and calculate the percentage of missing values (reused from MDA)
sg_data <- steam_games %>%
  mutate(all_reviews = ifelse(all_reviews == "NaN", NA, all_reviews)) %>% 
  ...
  mutate(
    ...
    positive_percent = as.numeric(positive_percent), 
    year = as.numeric(format(as.Date(release_date, format = "%b %d, %Y"), "%Y")) 
  )
...
#User Interface
ui <- fluidPage(
  titlePanel("Gaming Meets Data: Steam Insights"),
